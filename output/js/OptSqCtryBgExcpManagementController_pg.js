function OptSqCtryBgExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	var pgOptions = {moduleName : 'optSqCtryBgExcp'};
   	//save filter value after click OK button
	var oldFilterObj = {};
	//will update select options based on attrList(keys of $scope.select)
	var attrList = ["rgnCd", "subregn1Cd", "subregn2Cd", "subregn3Cd", "cntryCd", "busGrp", "note", "lastModById", "lastModTs"];
	var forDisplayList = ["rgnCdForDisplay", "subregn1CdForDisplay", "subregn2CdForDisplay", "subregn3CdForDisplay", "cntryCdForDisplay", "busGrp", "note", "lastModByEmail", "lastModTs"];
	//finished ajax call
	var count = 0;
	...
	// handle Broadcast from shareService when optSqCtryBgExcpList is update in other controller
	$scope.$on('handleGridDataBroadcast', function(scope,action,result) {
		if(action == 'optSqCtryBgExcp')
		{
	...
	$scope.add = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptSqCtryBgExcpManagementAdd']);
		logger.info("Executing add()");
		$modal
		.open({templateUrl: "html/OptSqCtryBgExcpEdit.html", controller: OptSqCtryBgExcpEditCtrl,
							windowClass : 'oneCol',							keyboard: false,
							resolve: {selectedOptSqCtryBgExcp: null} });
	}
	$scope.update = function () {
		...
		// Track user action
		_waq.push(['Click', 'OptSqCtryBgExcpManagementUpdate']);
		...
		$modal.open({templateUrl: "html/OptSqCtryBgExcpEdit.html", controller: OptSqCtryBgExcpEditCtrl,
																keyboard: false,
								resolve: {selectedOptSqCtryBgExcp: function() {return guidCopy;} } });
	};
	...
	$scope.del = function(){
		...
		// Track user action
		_waq.push(['Click', 'OptSqCtryBgExcpManagementDelete']);
		// get selected from scope
		var selectedItem = $scope.selection;
		if (selectedItem.length < 1) {
		...
		confirm.confirm(AdminService.getTipText('confirmDelete'),function(){
			...
			AdminService.deleteOptSqCtryBgExcp(options,function(result){
				// update optSqCtryBgExcpList when delete completed
				...
				if(result.totalRecordAmount > 0 && result.lineData.length == 0){
					...
					AdminService.getOptSqCtryBgExcpFilteredData(para,function(result) {
						$scope.currentPage = 1;
						pgOptions.data = result.lineData;
						pgOptions.totalSize = result.totalRecordAmount;
			            serverSidePaging(AdminService,loading,confirm,$scope,$filter,pgOptions);
	...
	$scope.exportAsExcel = function(partFlag) {
		...
		//gridDataToJson(setting, 'PQB Country/BG Exclusion');
		//for OptSqCtryBgExcp page only ["lastModById"] is bigint
		...
		var entityName = 'OptSqCtryBgExcp';
	...
	AdminService.getOptSqCtryBgExcpData(para,function(result) {
		AdminService.getStaticData($scope.setAuth,plusOne);
		$scope.headers = [
					{title: '',value: '',cd: '',visible: true,className: ''},
					{title: 'Region',value: 'rgnCdForDisplay',cd: 'rgnCd',visible: true,className: 'pg-col-width'},
					{title: 'Sub Region 1',value: 'subregn1CdForDisplay',cd: 'subregn1Cd',visible: false,className: 'pg-col-width'},
					{title: 'Sub Region 2',value: 'subregn2CdForDisplay',cd: 'subregn2Cd',visible: false,className: 'pg-col-width'},
					{title: 'Sub Region 3',value: 'subregn3CdForDisplay',cd: 'subregn3Cd',visible: false,className: 'pg-col-width'},
					{title: 'Country',value: 'cntryCdForDisplay',cd: 'cntryCd',visible: true,className: 'pg-col-width'},
					{title: 'Business Group',value: 'busGrp',cd: 'busGrp',visible: true,className: 'pg-col-width'},
	...
	$scope.getFilterData = function(){
		...
		AdminService.getOptSqCtryBgExcpSelectData(function(result) {
